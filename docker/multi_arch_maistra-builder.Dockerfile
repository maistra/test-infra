...
# Switch dependencies among different arches
ENV ARCH
RUN case $(arch) in \
        x86_64) \
            ARCH="amd64"; \
...
RUN dnf -y install dnf-plugins-core && \
    ...
    dnf -y install --setopt=install_weak_deps=False \
                   ...
                   clang-0:15.0.7-1.module+el8.8.0+17939+b58878af llvm-0:15.0.7-1.module+el8.8.0+17939+b58878af lldb-0:15.0.7-1.module+el8.8.0+17939+b58878af compiler-rt-0:15.0.7-1.module+el8.8.0+17939+b58878af \
                   docker-ce docker-buildx-plugin npm yarn rpm-build

# Replace dnf install ninja-build with pip3 install ninja
# Python tools
...
# YQ
RUN curl -sfL https://github.com/mikefarah/yq/releases/download/v${YQ_VERSION}/yq_linux_${ARCH} -o /usr/local/bin/yq-go && chmod +x /usr/local/bin/yq-go

# GH CLI
RUN curl -sfLO https://github.com/cli/cli/releases/download/v${GH_VERSION}/gh_${GH_VERSION}_linux_${ARCH}.tar.gz && \
...
# Helm
RUN curl -sfL https://get.helm.sh/helm-${HELM3_VERSION}-linux-${ARCH}.tar.gz | tar -xz linux-${ARCH}/helm --strip=1 && \
    mv helm /usr/local/bin/helm && chown root.root /usr/local/bin/helm && ln -s /usr/local/bin/helm /usr/local/bin/helm3

# Kubectl
RUN curl -sfL https://storage.googleapis.com/kubernetes-release/release/v${KUBECTL_VERSION}/bin/linux/${ARCH}/kubectl -o /usr/local/bin/kubectl && \
...
# Promu
RUN curl -sfLO https://github.com/prometheus/promu/releases/download/v${PROMU_VERSION}/promu-${PROMU_VERSION}.linux-${ARCH}.tar.gz && \

# Maistra Development Environment in Docker

This `Dockerfile` creates an ubi8 Docker image for developing on Istio.
...
The first time this target it run, a Docker image named `maistra/dev:USER` is created, where USER is your local username.
Any subsequent run won't rebuild the image unless the `Dockerfile` is modified.

The first time this target is run, a container named `maistra-dev` is run with this image, and an interactive shell is executed in the container.

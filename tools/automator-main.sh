...
get_opts() {
  ...
  while true; do
    ...
    --merge-branch)
      ...
      shift 2
      ;;
    --merge-failure-notify)
      merge_failure_notify="$2"
      shift 2
      ;;
    --merge-failure-label)
...
validate_opts() {
  ...
  if [ -n "${merge_failure_notify:-}" ]; then
    merge_failure_notify="cc $merge_failure_notify"
  fi

  if [ -n "${merge_failure_label:-}" ]; then
    merge_failure_label="-l $merge_failure_label"
  fi
